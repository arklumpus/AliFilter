...
namespace Figure_S11
{
    internal partial class Program
    {
        ...
        static Page CreateFigureS11c(bool useCache = true)
        {
            ...
            {
                { "reference", ( new double[] { -1000, -24 }, new double[] { -1050, -31 } ) },
                { "simulated", ( new double[] { 500, 15 }, new double[] { 1000, 14 } ) },
                { "raw", (new double[] { 1250, -22 }, new double[] { 1350, -10 }) },
                { "alifilter", (new double[] { -20, -18 }, new double[] { -450, -7 }) },
                { "bmge", (new double[] { -800, 10 }, new double[]{ -1200, 3 }) },
                { "trimal", (new double[] { -1200, -14 }, new double[] { -1250, -5 }) },
                { "gblocks", (new double[] { -1400, 16 }, new double[] { -1750, 6 }) },
                { "noisy", (new double[] { 500, 1 }, new double[] { 900, 0 }) },
                { "clipkit", (new double[] { 800, -15 }, new double[] { 1200, -6 }) }
            };
            ...
            plot.AddPlotElement(new PlotElement<IReadOnlyList<double>>(plot.GetFirst<IContinuousCoordinateSystem>(), (gpr, coord) =>
            {
                ...
                for (int i = 0; i < tools.Length - 1; i++)
                {
                    ...
                    {
                        Point pt = coord.ToPlotCoordinates(treeCoordinates[(tools.Length - 1) * replicates + 1 + i * sampleSize + j]);
                ...
                {
                    for (int j = 0; j < (i < tools.Length - 1 ? replicates : 1); j++)
                    {
        ...
        static double[][] GetTreeCoordinatesFrobenius(bool useCache, string[] tools, int sampleSize, int replicates)
        {
            ...
            if (!useCache || !File.Exists("Cache/FigureS11c_coordinates.txt"))
            {
                float[][] distanceMatrixOfTrees;

                if (!useCache || !File.Exists("Cache/FigureS11c_distMat.bin"))
                {
                    TreeNode[][] subsampledTrees;

                    if (!useCache || !File.Exists("Cache/FigureS11c_raw.tbi"))
                    {
                        ...
                        if (useCache)
                        {
                            ...
                            for (int i = 0; i < tools.Length; i++)
                            {
                                BinaryTree.WriteAllTrees(subsampledTrees[i], "Cache/FigureS11c_" + tools[i] + ".tbi");
                            }
                        }
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < tools.Length; i++)
                        {
                            subsampledTrees[i] = BinaryTree.ParseAllTrees("Cache/FigureS11c_" + tools[i] + ".tbi").ToArray();
                        }
                    }
                    ...
                    // Read the ML trees.
                    TreeNode[][] mlTrees = ReadMLTrees(tools, true);
                    ...
                    if (useCache)
                    {
                        ...
                        SaveDistanceMatrix("Cache/FigureS11c_distMat.bin", distanceMatrixOfTrees);
                    }
                }
                else
                {
                    // Reuse the cached distance matrix of trees.
                    distanceMatrixOfTrees = ReadDistanceMatrix("Cache/FigureS11c_distMat.bin");
                }
                ...
                if (useCache)
                {
                    ...
                    SaveTreeCoordinates("Cache/FigureS11c_coordinates.txt", treeCoordinates);
                }
            }
            else
            {
                // Reuse the cached coordinates.
                treeCoordinates = ReadTreeCoordinates("Cache/FigureS11c_coordinates.txt");
            }
