...
namespace Figure_S11
{
    internal partial class Program
    {
        static void Main(string[] args)
        {
            ...
            Page figureS11a = CreateFigureS11a();
            Page figureS11c = CreateFigureS11b();
            Page figureS11b = CreateFigureS11c();

            Font partLetterFont = new Font(FontFamily.ResolveFontFamily(FontFamily.StandardFontFamilies.HelveticaBold), 20);

            Page figureS11 = new Page(1, 1);

            figureS11.Graphics.FillText(0, 0, "a)", partLetterFont, Colours.Black);
            figureS11.Graphics.DrawGraphics(0, 0, figureS11a.Graphics);

            figureS11.Graphics.FillText(figureS11a.Width + 5, 0, "b)", partLetterFont, Colours.Black);
            figureS11.Graphics.DrawGraphics(figureS11a.Width, 0, figureS11b.Graphics);

            figureS11.Graphics.FillText(0, figureS11a.Height + 10, "c)", partLetterFont, Colours.Black);
            figureS11.Graphics.DrawGraphics(0, figureS11a.Height + 10, figureS11c.Graphics);

            figureS11.Crop();

            // Resize to a width of 17cm.
            Page finalFigureS11 = new Page(482, figureS11.Height * 482 / figureS11.Width);
            finalFigureS11.Background = Colours.White;
            finalFigureS11.Graphics.Scale(482 / figureS11.Width, 482 / figureS11.Width);
            finalFigureS11.Graphics.DrawGraphics(0, 0, figureS11.Graphics);

            Document doc = new Document();
            doc.Pages.Add(finalFigureS11);

            finalFigureS11.SaveAsSVG("Figure_S11.svg");
            finalFigureS11.SaveAsSVG("Figure_S11.notext.svg", SVGContextInterpreter.TextOptions.ConvertIntoPathsUsingGlyphs);
            doc.SaveAsPDF("Figure_S11.pdf");
            finalFigureS11.SaveAsPNG("Figure_S11.png", 600.0 / 72);
        }

        static Page CreateFigureS11a()
        {
            ...
            Plot barChart = Plot.Create.StackedBarChart(new[] { ("", (IReadOnlyList<double>)new double[] { 0, 0, 125000 }) }.Concat(alignmentData.Select(x => (toolNames[x.toolName], (IReadOnlyList<double>)new double[] { x.parsInformative, x.distinct - x.parsInformative, x.totalLength - x.distinct }))).ToArray(), yAxisTitle: "Alignment length (residues)", width: 200);
            ...
            barChart.AddPlotElement(new DataLabels<IReadOnlyList<double>>(Enumerable.Range(0, 6).Select(x => new double[] { barChart.GetAll<ContinuousAxis>().ElementAt(1).StartPoint[0], x * 25000 }), barChart.GetFirst<IContinuousCoordinateSystem>())
            {
            ...
            barChart.AddPlotElement(new DataLabels<IReadOnlyList<double>>(alignmentData.Skip(2).Select((x, i) => new double[] { i + 3, x.totalLength }), barChart.GetFirst<IContinuousCoordinateSystem>())
            {
                Label = (_, x) => (x[1] / alignmentData[1].totalLength).ToString("0%"),
            ...
            barChart.AddPlotElement(new PlotElement<IReadOnlyList<double>>(barChart.GetFirst<IContinuousCoordinateSystem>(), (gpr, coord) =>
            {
                ...
                gpr.Rotate(-Math.PI / 2);
                gpr.FillText(0, 3, "Total", labelFont, BlendWithWhite(toolColours[alignmentData[0].toolName], 0.33), TextBaselines.Baseline);
                gpr.Restore();
