...
namespace Figure_S6
{
    partial class Program
    {
        static Page CreateFigureS6b()
        {
            ...
            double minRAM = 500.0 * 1024 * 1024;
            double maxRAM = 320.0 * 1024 * 1024 * 1024;

            double minSize = 200.0 * 1024 * 1024;
            double maxSize = 30.0 * 1024 * 1024 * 1024;

            // Use a logarithmic coordinate system.
            LogarithmicCoordinateSystem2D coordinateSystem = new LogarithmicCoordinateSystem2D(minSize, maxSize, minRAM, maxRAM, 250, 250);

            // Create the initial plot.
            Plot plot = Plot.Create.ScatterPlot(new double[][] { new double[] { minSize, minRAM }, new double[] { maxSize, maxRAM } }, coordinateSystem: coordinateSystem, xAxisTitle: "Alignment file size", yAxisTitle: "Maximum RAM usage");
            ...
            plot.AddPlotElement(new PlotElement<IReadOnlyList<double>>((ICoordinateSystem<IReadOnlyList<double>>)plotElements[0].CoordinateSystem, (gpr, coord) =>
            {
                ...
                double[] yTicks = new double[] { 500, 1024, 2 * 1024, 5 * 1024, 10 * 1024, 20 * 1024, 50 * 1024, 100 * 1024, 200 * 1024 };
                string[] yLabels = new string[] { "500MiB", "1GiB", "2GiB", "5GiB", "10GiB", "20GiB", "50GiB", "100GiB", "200GiB" };

                double[] yIntervals = Enumerable.Range(5, 5).Select(x => x * 100.0).Concat(Enumerable.Range(1, 9).Select(x => x * 1024.0)).Concat(Enumerable.Range(1, 9).Select(x => x * 10240.0)).Concat(Enumerable.Range(1, 3).Select(x => x * 102400.0)).ToArray().ToArray();
                ...
                for (int i = 0; i < yTicks.Length; i++)
                {
                    ...
                    p1 = coord.ToPlotCoordinates(new double[] { topLeft[0], yTicks[i] * 1024 * 1024 });
                    p2 = coord.ToPlotCoordinates(new double[] { bottomRight[0], yTicks[i] * 1024 * 1024 });
                ...
                for (int i = 0; i < yIntervals.Length; i++)
                {
                    ...
                    p1 = coord.ToPlotCoordinates(new double[] { topLeft[0], yIntervals[i] * 1024 * 1024 });
                    p2 = coord.ToPlotCoordinates(new double[] { bottomRight[0], yIntervals[i] * 1024 * 1024 });
            ...
            plot.AddPlotElement(new PlotElement<IReadOnlyList<double>>(plot.GetFirst<IContinuousCoordinateSystem>(), (gpr, coord) =>
            {
                for (int i = 0; i < tools.Length; i++)
                {
                    ...
                    {
                        Point pt = coord.ToPlotCoordinates(new double[] { AlignmentData[kvp.Key].fileSize, kvp.Value.ram.Select(x => (double)x).Median() });
...

                {
                    ...
                    double maxY = benchmarkResults.SelectMany(x => x.Where(y => y.Key != "LSU" && y.Key != "SSU").Select(x => x.Value.ram.Max())).Max();

                    Point p1 = coord.ToPlotCoordinates(new double[] { minX, maxY });
                    Point p2 = coord.ToPlotCoordinates(new double[] { maxX, maxY });

                    gpr.StrokePath(new GraphicsPath().MoveTo(p1 + new Point(-7, 3)).LineTo(p1 + new Point(-7, -7)).LineTo(p2 + new Point(7, -7)).LineTo(p2 + new Point(7, 3)), Colours.Black);
...

                {
                    ...
                    double minYLSU = benchmarkResults.SelectMany(x => x.Where(y => y.Key == "LSU").Select(x => x.Value.ram.Min())).Min();
                    double minYSSU = benchmarkResults.SelectMany(x => x.Where(y => y.Key == "SSU").Select(x => x.Value.ram.Min())).Min();
