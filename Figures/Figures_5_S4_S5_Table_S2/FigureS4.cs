...
namespace Figures_5_S4_S5_Table_S2
{
    internal static partial class Program
    {
        static void CreateFigureS4(bool useCache = true)
        {
            ...
            {
                { "raw", new double[][]{ new double[] { -8.5, 13 }, new double[] { -7.5, 0.5 } } },
                { "alifilter", new double[][]{ new double[] { 6.2, 13 }, new double[] { -6.4, 0 } } },
                { "bmge", new double[][]{ new double[] { -4, -12.5 }, new double[]{ -1, -9 } } },
                { "trimal", new double[][]{ new double[] { 16, -12.7 }, new double[]{ 8.7, -6.1 } } },
                { "gblocks", new double[][]{ new double[] { 29.5, -4 }, new double[]{ 23.8, -1 }, new double[] { 26.5, 5.2 }, new double[] { 24.9, 5.1 } } },
                { "noisy", new double[][] { new double[] { -14.5, -2.5 }, new double[] { -11.3, 1.5 }, new double[] { -8.5, -1.2 } } },
                { "clipkit", new double[][]{ new double[] { -9.6, -5.5 }, new double[] { -7.9, -2 } } },
                { "gb", new double[][]{ new double[] { -5.5, -8.6 }, new double[] { -7.1, -2.2 } } }
            };
            ...
            plot.AddPlotElement(new PlotElement<IReadOnlyList<double>>(plot.GetFirst<IContinuousCoordinateSystem>(), (gpr, coord) =>
            {
                ...
                for (int i = 0; i < tools.Length; i++)
                {
                    for (int j = 0; j < replicates; j++)
                    {
                        ...
                        if (tools[i] == "clipkit" || tools[i] == "alifilter" || tools[i] == "raw" || (tools[i] == "noisy" && j != 2) || tools[i] == "gb")
                        {
